    ...
    <Shell>
      <div className="flex flex-col gap-8 max-w-4xl mx-auto py-8">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-6 relative">
            ...
            {history.map((item) => (
              <div key={item.id} className="relative flex items-stretch md:items-center gap-6 group">
                <div className="hidden md:flex w-12 h-12 rounded-full glass-dark border-2 border-white/30 z-10 items-center justify-center shrink-0">
                  <div className="w-3 h-3 rounded-full bg-white/70 group-hover:bg-white transition-colors"></div>
                </div>

                <div className="flex-1 glass-dark rounded-3xl p-6 transition-all hover:border-primary/30 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                  ...
                  <div className="flex-1 sm:flex-[0.8] flex items-center justify-between glass-dark p-4 rounded-2xl w-full sm:w-auto">
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-white/80 uppercase tracking-wider mb-1">Recommended</span>
                      <span className="font-bold text-foreground text-lg line-clamp-1">{item.restaurant.name}</span>
                    </div>
                    <Link href={`/restaurant/${item.restaurant.id}`} className="w-10 h-10 rounded-full glass-dark flex items-center justify-center shrink-0 text-foreground hover:bg-primary hover:text-primary-foreground transition-colors">
          ...
          <div className="flex flex-col items-center justify-center text-center p-12 glass-dark border-2 border-dashed border-white/30 rounded-3xl mt-8">
            ...
            <p className="text-white/80 font-medium mb-6">Ask for a recommendation and it will show up here.</p>
