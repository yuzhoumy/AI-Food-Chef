  ...
  return (
    <Shell>
      <div className="flex flex-col gap-8 max-w-4xl mx-auto py-8">
        ...
        <div className="flex flex-col gap-2 glass rounded-3xl p-6">
        ...
        {isLoading ? (
          <div className="flex flex-col gap-6">
            {[1, 2, 3].map(i => (
              <Skeleton key={i} className="h-32 w-full rounded-3xl glass" />
            ))}
          </div>
          ...
          <div className="flex flex-col gap-6 relative">
            ...
            {history.map((item) => (
              <div key={item.id} className="relative flex items-stretch md:items-center gap-6 group">
                <div className="hidden md:flex w-12 h-12 rounded-full glass border-2 border-white/30 z-10 items-center justify-center shrink-0">
                  <div className="w-3 h-3 rounded-full bg-white/70 group-hover:bg-white transition-colors"></div>
                </div>

                <div className="flex-1 glass rounded-3xl p-6 transition-all hover:border-primary/30 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                  ...
                  <div className="flex-1 sm:flex-[0.8] flex items-center justify-between glass p-4 rounded-2xl w-full sm:w-auto">
                    ...
                    <Link href={`/restaurant/${item.restaurant.id}`} className="w-10 h-10 rounded-full glass flex items-center justify-center shrink-0 text-white hover:bg-primary hover:text-primary-foreground transition-colors">
          ...
          <div className="flex flex-col items-center justify-center text-center p-12 glass border-2 border-dashed border-white/30 rounded-3xl mt-8">
